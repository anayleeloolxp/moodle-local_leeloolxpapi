...
class local_leeloolxpapi_external extends external_api {
    ...
    public static function course_sync($reqcoursedata = '', $reqcategoriesdata = '', $reqgradedata = '') {
        ...
        $reqdataall = (object) json_decode($reqcoursedata, true);
        if (!empty($reqdataall->hierarchy_data)) {

            $structuredata = json_decode($reqdataall->hierarchy_data);
            $courseid = json_decode($reqdataall->courseid);

            $returnhierarchyjson = self::save_sections_ars($structuredata, $courseid);

            if (!empty($returnhierarchyjson)) {

                return $returnhierarchyjson;
            }
            return '';
        } else {
            $value = (object) json_decode($reqcoursedata, true);
            ...
            $isinsert = 1;
            $checkshort = 1;
            if (!empty($value->is_clone_course)) {
                $checkshort = 0;
            }
            ...
            if ($isinsert) {
                ...
                $lastid = $DB->insert_record('course_sections', $tempdata);
            } else {
    ...
    /**
     * Save Sections and ARs
     *
     * @param string $req_arr req_arr will have single skillset
     */
    public static function save_sections_ars($reqarrall, $courseid) {

        global $DB;
        $structuredata = (object)$reqarrall;
        ...
        if (!empty($skillsets)) {
            ...
            $qcdata = $DB->get_record_sql(
                "SELECT section FROM {course_sections} where course = '$courseid' ORDER BY section DESC"
            );

            if (!empty($qcdata)) {
                $sectionorder = $qcdata->section;
            } else {
                $sectionorder = 0;
            }
            ...
            foreach ($skillsets as $keyss => $valuess) {
                if (!empty($valuess)) {
                    ...
                    $tempobject->sectionid = $sectionid;
                    ...
                    if (!empty($skills[$keyss])) {
                        foreach ($skills[$keyss] as $keyskills => $valueskills) {
                            if (!empty($valueskills)) {
                                ...
                                if (!empty($structuredata->skillstaskid[$keyss][$keyskills]->tasks)) {
                                    foreach ($structuredata->skillstaskid[$keyss][$keyskills]->tasks as $keytemp => $taskdata) {
                                        $taskdata = (array)$taskdata;
                                        $taskdata['course_id'] = $courseid;
                                            ...
                                            if (!empty($taskdata['activity_id'])) {
                                                if (!empty($taskdata['page_content'])) {
                                                    ...
                                                    foreach ($taskdata['page_content'] as $keytempar => $valuetemparrr) {
                                                        $valuetemparrr = (array)$valuetemparrr;
                                        ...
                                        $questiondata = (object)$taskdata;
                                        $keyactivity = '0';
                                        if (!empty($questiondata->questions_data[$keyactivity])) {
                                            $valueactivity = $taskdata;
                                                ...
                                                if (!empty($contextdata)) {
                                                    ...
                                                    if (!empty($qcdata)) {
                                                        ...
                                                        if (!empty($quizdataa)) {
                                                            ...
                                                            foreach ($questionsdata as $keyssques => $valuess) {
                                                                $valuess = (array)$valuess;
                                                                $sectiondata = array();
                                                                ...
                                                                if (!empty($quizslotsdatatotal->total)) {
                                                                ...
                                                                if (!empty($valuess['answers'])) {
                                                                    ...
                                                                    foreach ($valuess['answers'] as $keyanswers => $valueanswers) {
                                                                        $valueanswers = (array)$valueanswers;
                                                                        $answersdata = array();
                                ...
                                if (!empty($units[$keyss][$keyskills])) {
                                    foreach ($units[$keyss][$keyskills] as $keyunits => $valueunits) {
                                        if (!empty($valueunits)) {
                                            ...
                                            if (!empty($structuredata->unitstaskid[$keyss][$keyskills][$keyunits]->tasks)) {
                                                foreach ($structuredata->unitstaskid[$keyss][$keyskills][$keyunits]->tasks as $keytemp => $taskdata) {
                                                    $taskdata = (array)$taskdata;
                                                    $taskdata['course_id'] = $courseid;
                                                    if ($taskdata['task_type'] == 'page') {
                                                        if (!empty($taskdata['activity_id'])) {
     ...
     * @return external_function_parameters
     */
    public static function fetch_category_courses_parameters() {
        return new external_function_parameters(
            array(
                'category_id' => new external_value(PARAM_RAW, 'category_id', VALUE_DEFAULT, null),
            )
        );
    }
     ...
     * @return string welcome message
     */
    public static function fetch_category_courses($categoryid = '') {

        global $DB, $CFG;
        // Parameter validation.
        // REQUIRED.
        $params = self::validate_parameters(
            self::fetch_category_courses_parameters(),
            array(
                'category_id' => $categoryid,
            )
        );

        $categoryidarr = json_decode($categoryid, true);
        $tempArr = $categoryidarr[0];
        $tempArr = explode('&', $tempArr);
        $categoryid = $tempArr[0] . '"';
        $tempVal = $tempArr[1];
        if (strpos($tempVal, '1') !== false) {
                                        ...
                                        foreach ($moduledetail as $key => $valuefinal) {
                                            ...
                                            if ($cm) {
                                                if ($cm->modname == 'quiz') {
                                                    ...
                                                    $iconurl = $iconurl->get_scheme() . '://' . $iconurl->get_host() . '' . $iconurl->get_path();
                                                }
                                            ...
                                            $querystring = $coursedetails->fullname . "$$" .
                                                ...
                                                base64_encode(
                                                    json_encode(
                                                        array_values($moduledetail)[0]
                                                    )
                                                );

                                            $valuefinal->iconurl = $iconurl;
                                            $valuefinal->activityid = $activityids->id;
                                            $valuefinal->querystring = $querystring;
                                            ...
                                            if (isset($valuefinal->questionsperpage)) {
                                                ...
                                                if (!empty($isquiz)) {
                                                    $valuefinal->check_quiz_sync = $querystring;
                                                }
                                            }
                                            $actvitiesArr[$sectionsdetails->id][] = $valuefinal;
                                        }
                                    }
                                }
                            }
     ...
     * @return external_description
     */
    public static function fetch_category_courses_returns() {
        return new external_value(PARAM_TEXT, 'Returns data');
    }
     ...
     * @return external_function_parameters
     */
    public static function sync_course_activities_parameters() {
        return new external_function_parameters(
            array(
                'data_array' => new external_value(PARAM_RAW, 'data_array', VALUE_DEFAULT, null),
                'type' => new external_value(PARAM_RAW, 'type', VALUE_DEFAULT, null),
            )
        );
    }
     ...
     * Sync course from Leeloo to Moodle
     *
     * @param string $data_array data_array
     * @return string welcome message
     */
    public static function sync_course_activities($data_array = '') {

        global $DB, $CFG;
        // Parameter validation.
        // REQUIRED.
        $params = self::validate_parameters(
            self::sync_course_activities_parameters(),
            array(
        ...
        if (!empty($dataArray[0])) {
            ...
            foreach ($alldata as $key => $value) {
                ...
                if ($CFG->dbtype == 'mysqli') {
                    ...
                    on cfo.sectionid = cs.id and cfo.name = 'parent' WHERE cs.course = ? and cs.section != '0'
                    ...
                    WHERE course = ? and cs.section != '0' ORDER BY cs.id ASC";
                    $coursehierarchy = $DB->get_records_sql($sql, [$courseidagain, $courseidagain]);
        ...
        return json_encode($dataArray[0]);
    }
     ...
     * @return external_description
     */
    public static function sync_course_activities_returns() {
        return new external_value(PARAM_TEXT, 'Returns data');
    }
     ...
     * @return external_function_parameters
     */
    public static function resync_activity_course_parameters() {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_RAW, 'id', VALUE_DEFAULT, null),
                'action' => new external_value(PARAM_RAW, 'action', VALUE_DEFAULT, null),
                'type' => new external_value(PARAM_RAW, 'type', VALUE_DEFAULT, null),
            )
        );
    }
     ...
     * @return string welcome message
     */
    public static function resync_activity_course($id = '', $action = '', $type = '') {

        global $DB, $CFG;
        // Parameter validation.
        // REQUIRED.
        $params = self::validate_parameters(
            self::resync_activity_course_parameters(),
            array(
                'id' => $id,
                'action' => $action,
                'type' => $type,
        ...
        if ($type  == '1' || $type  == 1) { // AR
            if ($action == 'resync') {
                $reactivityid = $activityid = $paramsArr['id'];
                $activityname = $reqactivityname = $paramsArr['activity_name'];
                $courseid = $reqcourseid = $paramsArr['course_id'];
                ...
                $activityidmoodlearr = $requnsyncid = $paramsArr['id'];
            ...
            if ($action == 'resync') {
                if (!empty($courseid)) {
                    $courseidresync = $reqcourseidresync = $courseid;
     ...
     * @return external_description
     */
    public static function resync_activity_course_returns() {
        return new external_value(PARAM_TEXT, 'Returns data');
    }
     ...
     * @return external_function_parameters
     */
    public static function sync_activity_section_parameters() {
        return new external_function_parameters(
            array(
                'leeloodata' => new external_value(PARAM_RAW, 'leeloodata', VALUE_DEFAULT, null),
                'courseid' => new external_value(PARAM_RAW, 'courseid', VALUE_DEFAULT, null),
            )
        );
    }

    /**
     * Sync ars/section hierarchy from Leeloo to Moodle
     *
     * @param string $id id
     * @param string $action action
     * @param string $type type
     * @return string welcome message
     */
    public static function sync_activity_section($reqleeloodata = '', $courseid = '') {

        global $DB, $CFG;
        // Parameter validation.
        // REQUIRED.
        $params = self::validate_parameters(
            self::sync_activity_section_parameters(),
            array(
                'leeloodata' => $reqleeloodata,
                'courseid' => $courseid,
            )
        );

        $leeloodata = json_decode($reqleeloodata, true);
        ...
        $userid = $userdata->id;
        if (isset($leeloodata['email'])) {
            $email = (object) json_decode($leeloodata['email'], true);
            $userdata = $DB->get_record('user', ['email' => $email->scalar], 'id');
            if (!empty($userdata)) {
                $userid = $userdata->id;
            }
        ...
        if ($leeloodata['type'] == 'project') {

            if ($leeloodata['type'] == 'project') {
                $courseid = $leeloodata['courseid'];
                $structuredata = (object) json_decode($reqleeloodata, true);
                // Check if course entry exist , if not then insert one.
                $coursexist = $DB->get_record('course_sections', ['course' => $courseid]);
            ...
            if (!empty($leeloodata['questions_data'])) {
                $questiondata = (object) $leeloodata;
            }
     ...
     * @return external_description
     */
    public static function sync_activity_section_returns() {
        return new external_value(PARAM_TEXT, 'Returns data');
    }
}
